import oishiImg from '../assets/oishi.png';
import kimochiImg from '../assets/kimochi.png';
import aishiteruImg from '../assets/aishiteru.png';
import ichibanShabuImg from '../assets/ichiban_shabu.png';
import mabarAImg from '../assets/mabar_a.png';
import mabarCImg from '../assets/mabar_c.png';

export const menu = [
  { id: 1, name: "Oishi", desc: "Paket Shabu dan BBQ untuk 4 orang dengan alat.", price: "Rp 360.000", img: oishiImg,
    fullDesc: "\n"+
    "Beef lowfat 200gr\n"+
    "Beef fat 200gr\n"+
    "Fish ball mix 24 pcs\n"+
    "Ayam 100gr\n"+
    "Udang 120gr\n"+
    "Selada\n"+
    "Sayur mix (pakcoy + sawi)\n"+
    "Jamur mix (enoki + kuping)\n"+
    "Kuah 2 botol\n"+
    "Mie\n"+
    "Margarin\n"+
    "Cabai bubuk\n"+
    "Bawang putih + bawang bombay\n"+
    "Saus 3 botol\n"
   },
  { id: 2, name: "Kimochi", desc: "Paket BBQ saja untuk 4 orang dengan alat", price: "Rp 310.000", img: kimochiImg,
    fullDesc: "\n"+
    "Beef Lowfat : 200gr\n"+
    "Beef Fat : 200gr\n"+
    "Ayam 100 gr\n"+
    "Udang 120 gr\n"+
    "Saos 3 botol\n"+
    "Selada\n"+
    "Margarin + bawang putih + bombay\n"+
    "Alat :\n"+
    "1 kompor portable\n"+
    "1 grill + capitan\n"+
    "4 set alat makan barbeque\n"
   },
  { id: 3, name: "Aishiteru", desc: "Paket Shabu dan BBQ untuk 2 orang dengan alat", price: "Rp 237.000", img: aishiteruImg,
    fullDesc: "\n"+
    "Beef mix 150gr\n"+
    "Udang, ayam 100gr\n"+
    "Fish ball mix 12 pcs\n"+
    "Sayur mix\n"+
    "Jamur mix\n"+
    "Mie\n"+
    "Selada\n"+
    "Margarin\n"+
    "Bawang putih + bombay\n"+
    "Cabai bubuk\n"+
    "Sauce 1 pcs (boleh request bbq/bp/chilli oil)\n"+
    "Kuah 1 liter\n"+
    "\n"+
    "Alat Masak: \n"+
    "Kompor portable 2\n"+
    "Grill + Capitan\n"+
    "Panci kecil 1\n"+
    "Alat makan 2 set shabu & bbq\n"
   },
  { id: 4, name: "Ichiban Shabu", desc: "Paket Shabu untuk 4 orang dengan alat", price: "Rp 260,000", img: ichibanShabuImg, 
    fullDesc: "\n"+
      "Beef Lowfat 100gr\n"+
      "Fishball 24 pcs\n"+
      "Kuah 2 botol\n"+
      "Mie\n"+
      "Sayur mix (sawi + pakcoy)\n"+
      "Jamur mix (enoki + jamur kuping)\n"+
      "Cabe bubuk\n"
   },
  { id: 5, name: "Mabar A", desc: "Paket Shabu dan BBQ untuk 10 orang dengan alat", price: "Rp 545.000", img: mabarAImg,
    fullDesc: "\n"+
    "Ayam 400gr\n"+
    "Lowfat beef 400gr\n"+
    "Fat beef 200gr\n"+
    "Sosis 500gr\n"+
    "*Dimarinasi saus blackpepper*\n"+
    "Chilli Oil\n"+
        "\n"+
    "Selada\n"+
    "Mix jamur\n"+
    "Mix sayur\n"+
    "Mix fish ball 40pcs\n"+
    "Kuah 3 liter / btl\n"+
    "Mie\n"+
    "Cabe bubuk\n"+
    "Margarin\n"+
    "Bawang putih\n"+
        "\n"+
    "Alat masak (2 kompor, grill + panci)\n"+
    "Alat makan sekali pakai 10 set :\n"+
    "Paper bowl + sendok\n"+
    "Piring / pisin plastik + sumpit\n"
   },
  { id: 6, name: "Mabar C", desc: "Paket BBQ saja untuk 10 orang dengan alat", price: "Rp 370.000", img: mabarCImg,
    fullDesc: "\n"+
     "Ayam 400gr\n"+
     "Lowfat beef 400gr\n"+
     "Fat beef 200gr\n"+
     "Sosis 500gr\n"+
     "*Dimarinasi saus blackpepper*\n"+
     "+ Chilli Oil\n"+
     "Selada\n"+
     "Margarin \n"+
     "Bawang putih \n"+
     "  \n"+
     "Alat masak (kompor + grill)\n"+
     "Alat makan sekali pakai 10 set :\n"+
     "Piring / pisin plastik + sumpit\n"
  },
];
